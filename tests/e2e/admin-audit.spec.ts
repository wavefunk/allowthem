...
test("admin audit > event filter shows only matching events", async ({
  ...
  // Verify absence: no success-coloured event spans (login_failed is a failure event;
  // filtering to it must exclude all non-failure events rendered as span.text-gray-700).
  await expect(page.locator("td span.text-gray-700")).toHaveCount(0);
});
...
test("admin audit > outcome filter shows only failure events", async ({
  ...
  await expect(page.locator("td span.text-red-600").first()).toBeVisible();
  // Success-coloured spans (span.text-gray-700) must be absent — the filter
  // must exclude all success events from the result set.
  await expect(page.locator("td span.text-gray-700")).toHaveCount(0);
});
