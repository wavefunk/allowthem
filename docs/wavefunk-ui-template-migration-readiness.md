...
- `AuthShell` with splash slot, form slot, forced-mode attrs, and configurable
  asset base path.
- `AuthMain` or `AuthFormPanel` for the repeated right-column form structure.
- `AuthKicker`, `AuthTabs`, `OAuthProviderGrid`, and `TermsFooter` equivalents
  for the current MiniJinja `_auth_macros.html` behavior.
- HTMX-aware attributes on auth navigation links without requiring raw HTML in
  every consumer template.
...
- Typed `AuthSplash` or `ShaderAsciiSplash` component for iframe, image,
...
- `WorkspaceSwitcher` for `<details class="wf-workspace-switcher">`.
- `Sidenav`, `SidenavSection`, and `SidenavItem` for the current
  `wf-sidenav-*` structure, including muted and coming-soon states.
- A documented mapping from tenant roles to tag/badge variants.
...
- A reusable dynamic custom-field renderer may belong in AllowThem, not
  `wavefunk-ui`, because field schemas are AllowThem domain data.
...
- `ConsentScopeList` or a generic check-list display for OAuth scopes.
...
2. Add the missing auth shell primitives to `wavefunk-ui`.
3. Add app shell/modeline/workspace navigation primitives to `wavefunk-ui`.
4. Migrate core auth templates from MiniJinja HTML to Askama structs.
